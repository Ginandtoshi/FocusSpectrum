...
class Game3Scene(Scene):
    def __init__(self, manager):
        ...
        # Update dimensions to match screen
        global SCREEN_WIDTH, SCREEN_HEIGHT, GAME_WIDTH, GAME_HEIGHT, GAME_OFFSET_X, GAME_OFFSET_Y, BOTTOM_BAR_Y
        SCREEN_WIDTH, SCREEN_HEIGHT = pygame.display.get_surface().get_size()
        GAME_WIDTH = SCREEN_WIDTH - 2 * BORDER_SIZE
        GAME_HEIGHT = SCREEN_HEIGHT - 2 * BORDER_SIZE
        GAME_OFFSET_X = BORDER_SIZE
        GAME_OFFSET_Y = BORDER_SIZE
        BOTTOM_BAR_Y = GAME_OFFSET_Y + GAME_HEIGHT - BOTTOM_BAR_HEIGHT - PANEL_MARGIN
        
        # Load Font
        try:
            font_path = os.path.join(os.path.dirname(__file__), "../../../Asset/Algerian Regular.ttf")
            self.magnet_font = pygame.font.Font(font_path, 18)
            self.slot_font = pygame.font.Font(font_path, 20)
            self.ui_font = pygame.font.Font(font_path, 16)
            self.title_font = pygame.font.Font(font_path, 32)
            self.counter_font = pygame.font.Font(font_path, 28)
            self.distraction_font = pygame.font.Font(font_path, 14)
        except:
        ...
        self.exit_btn_rect = pygame.Rect(SCREEN_WIDTH - 120, SCREEN_HEIGHT - 60, 100, 40)
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if self.game_won:
                    ...
                    if pygame.time.get_ticks() - self.win_time > 2000:
                        # Save detailed stats
                        self.manager.data["scores"]["game3"] = {
                            "errors": self.error_count,
                            "time_taken": (self.end_time - self.start_time) / 1000.0,
                            "distraction_pct": self.distraction_percentage
                        }

                        if "game3" not in self.manager.data.get("completed_games", []):
                            if "completed_games" not in self.manager.data:
                                self.manager.data["completed_games"] = []
                            self.manager.data["completed_games"].append("game3")
    ...
    def update(self):
        ...
        # 更新眼动追踪
        frame = self.manager.camera.get_frame()
    ...
    def draw(self, screen):
        ...
        # Add a semi-transparent dark overlay to make the game elements pop
        overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
        overlay.fill((40, 10, 5, 180)) # Dark reddish tint with transparency
        screen.blit(overlay, (0, 0))
        
        # 绘制游戏区域边框（深色内嵌效果）
        game_area = pygame.Rect(GAME_OFFSET_X, GAME_OFFSET_Y, GAME_WIDTH, GAME_HEIGHT)
        pygame.draw.rect(screen, (50, 50, 50), game_area)
        pygame.draw.rect(screen, (100, 100, 100), game_area, 3)
        
        # 绘制边框区域的干扰元素
        current_time = pygame.time.get_ticks()
        self.border_distraction.draw(screen, current_time)
        ...
        pygame.draw.rect(screen, beige_bg, title_panel)
        
        # 深色边框
        border_color = (60, 50, 40)
        pygame.draw.rect(screen, border_color, title_panel, 4)
        ...
        pygame.draw.rect(screen, right_stripe_color,
        ...
        # L 在最左边（左侧橙棕色条内）
        screen.blit(l_text, (title_panel.x + (left_stripe_width - l_text.get_width()) // 2, 
                                  l_channel_rect.centery - l_text.get_height() // 2))
        # R 在最右边（右侧灰色条内）
        screen.blit(r_text, (title_panel.right - right_stripe_width + (right_stripe_width - r_text.get_width()) // 2,
        ...
        pygame.draw.line(screen, baseline_color, 
        ...
        screen.blit(title_text, (title_x, l_channel_rect.centery - title_text.get_height() // 2))
        ...
        screen.blit(rule_text, (rule_x, r_channel_rect.centery - rule_text.get_height() // 2))
        
        # 底部序列条面板
        bottom_bar = pygame.Rect(GAME_OFFSET_X + PANEL_MARGIN, BOTTOM_BAR_Y, 
                                GAME_WIDTH - 2 * PANEL_MARGIN, BOTTOM_BAR_HEIGHT)
        pygame.draw.rect(screen, RetroColors.PANEL_BG, bottom_bar)
        draw_panel_border(screen, bottom_bar, RetroColors.ACCENT_BLUE, 3)
        
        # 底部面板标题
        panel_label = self.ui_font.render("TARGET SEQUENCE", True, RetroColors.TEXT)
        screen.blit(panel_label, (bottom_bar.x + 10, bottom_bar.y + 10))
        
        # 进度信息（右上角）
        progress_text = self.ui_font.render(f"{self.current_target_index}/{len(self.slots)}", 
                                           True, RetroColors.ACCENT_TAN)
        screen.blit(progress_text, (bottom_bar.right - 70, bottom_bar.y + 10))
        ...
        # 绘制左侧卷轴
        pygame.draw.circle(screen, (40, 40, 40), (reel_left_x, reel_center_y), reel_radius)
        pygame.draw.circle(screen, (80, 80, 80), (reel_left_x, reel_center_y), reel_radius, 3)
        pygame.draw.circle(screen, (60, 60, 60), (reel_left_x, reel_center_y), reel_radius - 10)
        pygame.draw.circle(screen, (100, 100, 100), (reel_left_x, reel_center_y), reel_radius - 10, 2)
        ...
        for i in range(6):
            ...
            pygame.draw.line(screen, (120, 120, 120), line_start, line_end, 2)
        ...
        # 右侧卷轴（错误计数）
        pygame.draw.circle(screen, (40, 40, 40), (reel_right_x, reel_center_y), reel_radius)
        pygame.draw.circle(screen, (80, 80, 80), (reel_right_x, reel_center_y), reel_radius, 3)
        pygame.draw.circle(screen, (60, 60, 60), (reel_right_x, reel_center_y), reel_radius - 10)
        pygame.draw.circle(screen, (100, 100, 100), (reel_right_x, reel_center_y), reel_radius - 10, 2)
        ...
        screen.blit(time_label, (reel_left_x - time_label.get_width() // 2, reel_center_y + reel_radius + 5))
        
        error_label = label_font.render("ERRORS", True, RetroColors.TEXT_DIM)
        screen.blit(error_label, (reel_right_x - error_label.get_width() // 2, reel_center_y + reel_radius + 5))
        ...
        if len(self.slots) > 0:
            ...
            pygame.draw.rect(screen, RetroColors.ACCENT_TAN, 
        ...
        if self.game_started and self.total_frames > 30:
            ...
            pygame.draw.circle(screen, indicator_color, (indicator_x, distract_y), indicator_radius)
            pygame.draw.circle(screen, (200, 200, 200), (indicator_x, distract_y), indicator_radius, 2)
            ...
            screen.blit(distract_text, distract_rect)
        
        # 绘制槽位（在底部序列条中）
        current_time = pygame.time.get_ticks()
        for slot in self.slots:
            slot.draw(screen, self.current_target_index, current_time)
        
        # 绘制磁铁
        for magnet in self.magnets:
            if magnet != self.selected_magnet:
                magnet.draw(screen, False)
        
        if self.selected_magnet:
            self.selected_magnet.draw(screen, True)
        ...
        if self.eye_tracker.is_gaze_valid():
            ...
            screen.blit(gaze_surf, (gaze_x - gaze_size, gaze_y - gaze_size))
            
            # 显示校准状态
            status_text = self.distraction_font.render(self.eye_tracker.get_baseline_status(), 
                                                      True, RetroColors.TEXT_DIM)
            screen.blit(status_text, (GAME_OFFSET_X + PANEL_MARGIN + 20, 
